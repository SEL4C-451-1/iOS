...
class ActividadTerminadaViewController: UIViewController {
    override func viewDidLoad() {
        ...
        if self.restorationIdentifier == "Terminada1" {
            // Establecer el valor booleano en UserDefaults como true
            Task {
                let activity: ActivityResponse = try ActivityResponse(string_response: "complete")
                ...
                do{
                    try await activity.sendActivity()
                }catch{
                    do{
                        try await activity.sendActivityUpdate()
                    }catch{
                        showErrorAlert("Error mandando actividad.")
                    }
                }
            }
