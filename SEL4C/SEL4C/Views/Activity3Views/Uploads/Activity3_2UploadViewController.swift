...
class Activity3_2UploadViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        if let savedVideoURLString = UserDefaults.standard.string(forKey: "A32videoURL"), let savedVideoURL = URL(string: savedVideoURLString) {
        ...
        // Handle a movie capture
         UISaveVideoAtPathToSavedPhotosAlbum(
            ...
            #selector(video(_:didFinishSavingWithError:contextInfo:)),
             nil)
        videoURL = url
        UserDefaults.standard.set(videoURL?.absoluteString, forKey: "A32videoURL")
    }
